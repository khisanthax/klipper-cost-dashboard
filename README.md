...
- Runtime files: `data/settings.json` and `data/print_costs.csv` are created on first run from templates. If present, `data/settings.local.json` / `data/print_costs.local.csv` are used; otherwise the example files `data/settings.example.json` and `data/print_costs.example.csv` are used. Runtime files are local-only and not tracked in git.
